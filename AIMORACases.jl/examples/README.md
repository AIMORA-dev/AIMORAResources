...
The gallery contains 81 Julia-only examples in 81 named folders. Every catalogued folder owns a substantive `README.md`, `run.jl`, Makefile, and nonempty canonical `outputs/` tree. The committed result set contains 440 artifacts: 168 CSV data files, 144 SVG figures, 66 Markdown summaries, 39 text reports, 9 JSON summaries, 2 typed AIMORA checkpoints, and 12 TOML snapshots or summaries. No example contains a duplicate `results/` directory, workstation path, Fortran source, or compiled-reference runtime.
