...
The runner writes `local_multirate_partitioned_network.csv` with positive voltage, negative voltage, and oriented current for all seven interfaces at every accepted communication point. `local_partition_interface_voltage.svg` and `local_partition_interface_current.svg` provide curated views of the source-side, middle, and load-side boundaries. `local_partition_refinement.csv` and `local_partition_refinement.svg` show convergence from 40 and 20 microsecond communication windows toward the 10 microsecond equal-step limit. `summary.md` records region/rate/window work, the physical event, interface voltage/KCL/energy residuals, the passive two-region limit error, exact restart signatures, and unsupported claims.
