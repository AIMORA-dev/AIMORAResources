...
The gallery contains 60 Julia-only examples in 60 named folders. Every catalogued folder owns a substantive `README.md`, `run.jl`, Makefile, and nonempty canonical `outputs/` tree. The committed result set contains 311 artifacts: 117 CSV data files, 103 SVG figures, 51 Markdown summaries, 28 text reports, 9 JSON summaries, 2 typed AIMORA checkpoints, and 1 TOML summary. No example contains a duplicate `results/` directory, workstation path, Fortran source, or compiled-reference runtime.

The source-to-example audit indexes 470 discovered records: 9 public package testsets, 80 validation suites, 76 remaining validation fixtures, 55 canonical public case inputs consumed directly by validation, 16 classic public cases plus their shared provenance record, 11 implemented C301–C311 capability packets, 14 claimed but nonexistent C312–C325 packet designations, 93 official ATPDraw web cases, 52 bundled ATPDraw ACP projects/groups, and the claimed but unidentified DC-1–DC-63 collection. Of these records, 197 map to direct examples or explicit gallery variants; every other record has a named oracle, validation, negative-test, external-conversion, nonexistent-designation, or missing-source disposition.
