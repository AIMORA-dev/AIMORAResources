...
This reference describes AIMORA’s public model families, their engineering meaning, principal inputs, generated state, study use, outputs, and important limitations. Exact constructors and deck syntax remain versioned API contracts; this page explains when and why each family is used.
...
- **Lightning/impulse source:** parameterized current or voltage impulse used in surge studies; front and tail definitions must be stated.
...
| Family | Behavior |
| --- | --- |
| Timed ideal switch | changes topology at declared times |
| Current-zero switch | opens only after the admitted current-zero condition |
| Controlled switch | follows a typed control or TACS signal |
| Restrike/reignition switch | supports a declared voltage/current/event rule and localized discontinuity |
| Breaker sequence | coordinated pole operations, fault clearing, reclosing, trapped charge, and restart state |
...
- ZnO/surge-arrester characteristics;
...
| --- | --- |
| Average-value inverter | averaged converter dynamics without individual switching ripple |
| Two-level switching VSC | explicit valve states and switching events |
| Diode/thyristor bridge | topology and nonlinear valve conduction |
| Chopper | controlled DC switching topology |
| NPC and T-type converters | explicit multilevel switching states and neutral-point behavior |
| Flying-capacitor converter | capacitor-state and switching-sequence behavior |
| Cascaded H-bridge | cell-level or admitted aggregate switching representation |
| Semiconductor-fidelity extensions | declared conduction, reverse recovery, charge, or switching-loss behavior where qualified |
