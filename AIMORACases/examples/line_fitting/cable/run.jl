...
function main()
    ...
    source = line_parameter_example_source("public cable coupled line fitting")
    frequencies_hz = exp10.(range(0.0, 4.0; length=81))
    segment = example_cable_segment(
        source,
        example_soil(source);
        frequencies_hz,
    )
    uncertainty_source = line_parameter_example_source(
        "public cable coupled line fitting soil alternative",
    )
    uncertainty_segment = example_cable_segment(
        uncertainty_source,
